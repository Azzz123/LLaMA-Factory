...
def clean_text(text):
    ...
    if isinstance(text, str):
        ...
        if json_match:
            try:
                # 提取JSON数组内容
                json_array = json_match.group(1).strip()
                # 构造完整的causality_list格式
                json_str = f'{{"causality_list": {json_array}}}'
                # 验证JSON是否有效
                json.loads(json_str)
                return json_str
    ...
    try:
        ...
        special_tokens = [
            ...
            r'<｜User｜>',
            r'<｜Assistant｜>',
            r'<｜Think｜>.*?<｜Input｜>',  # 移除think过程
            r'<｜Think｜>.*?<｜Assistant｜>',  # 另一种think格式
            'Based on the input text, I extracted two events with causality relationships and formatted them according to your system definition:',
            'Let me analyze this step by step:',
            'Here\'s the extracted causality relationships in the required JSON format:',
        ]

        for token in special_tokens:
            text = re.sub(token, '', text, flags=re.DOTALL)
...
def evaluate(jsonl_path):
    ...
    # 保存结果
    output_path = os.path.join(os.path.dirname(jsonl_path), "evaluation_results_st.json")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        jsonl_file_path = "saves/DeepSeek/DeepSeek-R1-Distill-Llama-8B-cause/qlora/predict/0/generated_predictions.jsonl"
